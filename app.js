class HabitTracker {
    ...
    renderHabits() {
        ...
        this.habits.forEach((habit, index) => {
            ...
            // We use CSS variables for colors now, but we can still set a style for specific accent if needed
            // or just rely on the class if we had one. The current CSS uses --accent-color vars.
            // Let's keep the inline style for color but maybe we don't need it if we use classes?
            // Actually, the CSS defines --accent-blue etc.
            // Let's set a custom property on the card for the accent color to be used by children
            card.style.setProperty('--accent-color', `var(--accent-${habit.color})`);
            card.style.color = `var(--accent-${habit.color})`; // Keep this for text inheritance if needed
            ...
            card.innerHTML = `
                ...
                <div class="card-actions">
                    <button class="action-btn edit" aria-label="Edit habit">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                        </svg>
                    </button>
                    <button class="action-btn delete" aria-label="Delete habit">
    ...
    triggerConfetti(x, y) {
        const colors = ['#007AFF', '#FF3B30', '#34C759', '#FF9500', '#AF52DE', '#5856D6', '#5AC8FA'];
        for (let i = 0; i < 20; i++) {
